...
### 2. 📊 선수 검색, 1:1 비교 & 즐겨찾기 (Player Comparison & Favorites)
- **스탯 자동 계산**: 강화 등급(+1 ~ +10) 선택에 따른 스탯 가산치 자동 계산
- **듀얼 레이더 차트**: 두 선수의 6대 핵심 속성(PAC, SHO, PAS, DRI, DEF, PHY) 시각적 비교
- **상세 스탯 Side-by-Side 비교표**: 우세 능력치 하이라이트 글로우 및 스탯 차이 자동 표시
- **⭐ 즐겨찾기 선수(Favorites)**: 관심 선수를 로컬 스토리지에 영구 저장하고 홈 화면 및 검색 필터에서 빠르게 확인
